...
doc carries the design rationale. The configuration is one `Companion` object
per companion in `src/lib/companions/companions.ts`, where every field is
commented, so the type isn't repeated here.
...
here. OpenRouter fronts a wide range of hosted models, and each companion can
pick whichever one suits their persona, and swap it later, without standing up
any infrastructure.
...
The browser sees one origin, so there is no CORS handling. Streaming passes
straight through. The key never reaches the client.
...
and the companion is told the current date and time every turn.
...
Each export is commented with where it slots in, and each persona module
interpolates them into place, so those rules can't drift between companions.
...
Control of the toy is not among them. The shared block settles that for
everyone, so a persona written against it contradicts rather than overrides it.
Personas are written in the **second person** ("You're 21…") so they read as one
voice with the shared blocks.
...
commented at their declarations in `companions-panel/index.tsx`.
...
baseline** — low intensity, light variety, and a one-shot stroke-minus tease at
...
and running, and where the knobs sit, without a status tool. They stay in sync
even when a level is changed with the on-screen knobs rather than their own
tools.
...
thing that differs between one turn's request and the next. Putting them last
leaves everything before them — persona and whole conversation — identical, and
that is the shape prompt caching can reuse.
...
reaction. They see their own prior actions. Without that, the model drifts back
to narrating actions instead of taking them.
...
`wait_for_user` and go quiet until you speak. That keeps them from talking on at
someone who has stopped answering.

**How readily a companion fills a silence is their own.** A goonpack gives them
two separate settings, one for while the toy is idle and one for while it's
running. The appetites are separate: someone of few words can still keep up a
...
lightbox. What was sent is stored on the thread turn, so it survives a reload.
While the lightbox is open, a badge in its top corner shows every stage of a
turn, from you speaking to them speaking.

A companion with nothing to send gets neither tool, and is told outright that
they have nothing. Ask, and they say so rather than promising a picture that can
never arrive.
