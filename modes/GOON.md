...
a steady hold at the top. It models a long, unhurried arousal ramp to a
controlled climax — a "gooning" session, not deny-and-repeat edging.
...
you down a level (`Home › Goon › Play`) to the session itself, with the
transport, preview, stroke and intensity controls.

Setup is deliberately not changeable mid-play. **Reset** restarts the session
from time 0, staying put. `exit` — or the breadcrumb — climbs back up to setup,
and from there to home. Play needs the device connected, like Start.
...
build is fixed, and intensity scales evenly what actually reaches the device. At
the default **50**, the 25 → 100 build plays out as roughly **12 → 50%** on the
...
- **Ruin: eject** — drives the toy to push you out (a steady pace with the
  ...
  building the ejecting force takes a second or two of movement, which is
  stimulation at exactly the wrong moment, with voice-recognition lag on top. It
  can tip into a finish instead of a ruin. A ruin is timing-dependent even by
  hand.
...
Player's clock, which is what makes `forward`/`back`/`finish`/`faster`/`slower`
the Player moving or dilating that clock.
