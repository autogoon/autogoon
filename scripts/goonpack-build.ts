...
for (const dir of sourcesToBuild()) {
  ...
  await writeZip(dir, names, out, (done, total) =>
  ...
  // What parsePack accepted but the author probably didn't mean: pictures
  // still waiting for a sidecar, and names under media/ that nothing will play.
  const captions = captionWarning(parsed.undescribed);
  if (captions !== null) console.warn(yellow(`  warning: ${captions}`));
  const strays = strayWarning(parsed.strays);
  if (strays !== null) console.warn(yellow(`  warning: ${strays}`));
  built++;
}
