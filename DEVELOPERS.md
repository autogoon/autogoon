...
a range rather than to collect them. A pull request adding one is unlikely to be
accepted unless it fills a genuine gap in ethnicity or gender. If you think it
does, build it as a goonpack and submit that for consideration. Writing the
...
blocks its own cross-origin dev assets by default, which looks like a broken
app: the page renders, nothing is clickable. Set `DEV_ALLOWED_ORIGINS` in `.env`
...
- **Run the tests** (`npm test` and `npm run test:e2e` — see
  [Testing](#testing)). They are a floor, not the whole story. The app drives
...
- **End-to-end tests** — `npm run test:e2e` (Playwright, in `tests/e2e/`). Every
  ...
  server on :8931, or reuses one already running. The goonpack specs are the one
  exception, and skip themselves rather than being pinned to a browser: they
...
`MediaDevices.prototype` — instance assignment doesn't stick in WebKit) to
...
where your own content lives, per the [content policy](#content-policy) — with
the committed example pack `elise/` as the exception.
...
`describe` and `describe-missing` are vision-model work and so cover stills
only; a video's caption is written by hand.
