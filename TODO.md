...
**The next thing.** A companion with a large collected set can't use it: every
item's caption goes into the `send_media` schema and they pick by number, which
stops working long before the context window fills — a model choosing between
hundreds of near-identical captions chooses badly.
...
two texts per item, a summary of the set in the manifest, and two tools —
`search_media` returning a bounded working set of refs and captions,
`send_media` sending one by ref.
...
- **Phase 0 — one pack format.** Delete the format 1 compatibility path and
  renumber the current format to 1, leaving one accepted value. Independent of
  everything after it.

- **1 — two texts per item.** The long description stored beside the caption, as
  a `.md` sidecar carrying the caption in frontmatter and the description as the
  body. This is what makes caption iteration a re-condense of text already on
  disk rather than a fresh inference pass, so it comes before any of the
  roadmap's captioning work.

- **2 — the summary in the manifest**, generated by an npm script over the
  pack's own captions and descriptions.

- **3 — build and import validation.** Both texts required per media file, and a
  summary required of a pack that carries media. `goonpack-build.ts` runs
  `parsePack`, so one rule covers building and importing.

- **4 and 5 — the two tools**, landing together: `send_media` takes a ref, the
