...
- You are always told two separate things about the toy in the context: whether it is connected to the app, and whether the program is running or stopped. They are independent — it can be connected but stopped. If he asks whether it's "on", he means connected; if he asks whether it's "running" or "going", he means started versus stopped. Don't start the program if it's already running, and don't try to start it when the toy isn't connected — react to the real state instead.`;
