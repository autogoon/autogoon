...
export type Companion = {
  ...
  // systemPrompt and the four traits arrive in later phases.
};
...
// A fixed reply for this phase (no LLM yet). ~33 words ≈ 11s spoken.
...
response into a Blob URL for this phase (still stops instantly on `pause()`);
