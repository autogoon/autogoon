...
that write fails: `persistThread` logs it to the Companions event log, but
nothing recovers the turn, so the conversation rewinds to the last version that
fit on the next load.

To settle: which shape does it — summarising older turns, keeping a rolling
window of recent turns verbatim, or both — and what happens to the
`reasoning_details` belonging to the messages that go.
