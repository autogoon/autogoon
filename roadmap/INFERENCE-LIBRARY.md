...
two tools a companion calls — has shipped; [GOONPACKS.md](../GOONPACKS.md)
...
the set changes. Where it lives and when a pack must have one are in
[GOONPACKS.md](../GOONPACKS.md).

It has two jobs. The first is so they don't offer what isn't there —
proportions, who's in it, which acts appear, the settings, the range of undress.
The second is so they phrase a request in words the corpus actually uses, which
is what makes the search work: a request built from vocabulary the captions
don't contain matches nothing however good the retrieval is.

That makes it partly an inventory of terms — the hair colours, garments,
settings, acts and names present in the set — so its length follows the set
rather than a target. The line to hold is **enumerate the vocabulary, not the
...
It's a dull afternoon's work and nothing else under **Producing the
descriptions** should start without it — every comparison here is otherwise an
impression rather than a number, which is how the current prompt came to be
confidently wrong in a few places.
