...
holding sustained high arousal with undulation, rather than approaching and
backing off an edge.

Groove can approximate the gesture today (intensity full, dip variability off,
timing variability off → even 100 → 60 → 100 rolls, ~20 s per swell), which is
enough to test whether the sensation is worth chasing. But two things pin it
short of the real shape:
