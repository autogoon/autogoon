...
Read [ARCHITECTURE.md](./ARCHITECTURE.md) and [README.md](./README.md) for the full picture, and the per-algorithm docs ([ALGORITHM-GOON.md](./ALGORITHM-GOON.md), [ALGORITHM-HOMEGROWN.md](./ALGORITHM-HOMEGROWN.md), [ALGORITHM-AUTOPILOT.md](./ALGORITHM-AUTOPILOT.md)) before changing an algorithm. The cross-file things worth knowing up front:

- **Three layers per algorithm**: a plain-TS **engine** (`src/lib/*-engine.ts`, owns device commands + a subscribe/notify loop, no React), a **hook** (`src/hooks/use-*.ts`, mirrors the engine into render state and owns the UI defaults it constructs the engine with), and a **panel** (`src/components/*-panel.tsx`, presentation only). The engine knows nothing about the UI.
...
- **Engines are intentionally self-contained**: they do not import from each other. Goon deliberately duplicates Homegrown's `buildLeg` / `scaleSpeed` / cumming helpers rather than sharing a module — this is a chosen boundary, so don't refactor engines into a shared module without asking.
